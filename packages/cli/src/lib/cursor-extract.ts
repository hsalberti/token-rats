...
async function tryNodeSqlite(dbPath: string): Promise<CursorRow[] | null> {
  ...
  try {
    ...
    // biome-ignore lint/suspicious/noExplicitAny: node:sqlite types unavailable on older @types/node
...
async function tryBetterSqlite3(dbPath: string): Promise<CursorRow[] | null> {
  ...
  try {
    ...
    // biome-ignore lint/suspicious/noExplicitAny: better-sqlite3 default export shape varies across module systems
