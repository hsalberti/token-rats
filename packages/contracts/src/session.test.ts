/**
 * Back-compat guard for the SessionRecord wire contract.
 *
 * The CLI ships independently of the API, so an old binary keeps POSTing the
 * v0-shaped record (no `provider`, no `client`/`channel`, no granular cache /
 * reasoning token fields) long after newer fields land. These tests pin both
 * the v0 shape and the current shape so a future edit to the schema can't
 * silently make the optional fields required and start 400-ing old clients.
 */
