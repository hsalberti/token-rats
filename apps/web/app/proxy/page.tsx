      ...
      <div className="max-w-2xl mx-auto space-y-10">
        ...
        <div>
          ...
          <p className="mt-2 text-zinc-400">
            Use Token Rats as a drop-in proxy for the Anthropic API. Your usage
            gets logged automatically — no prompts or completions ever touch our
            storage.
          </p>
        </div>
        ...
        <section>
          ...
          <ul className="space-y-2 text-sm text-zinc-300">
            <li className="flex gap-2">
              ...
              <span>Forward your request bytes to <code className="bg-zinc-800 px-1 rounded text-xs">api.anthropic.com/v1/messages</code> unchanged.</span>
            </li>
            <li className="flex gap-2">
              <span className="text-green-400 mt-0.5 shrink-0">✓</span>
              <span>Read the <code className="bg-zinc-800 px-1 rounded text-xs">usage</code> field from the Anthropic response (input/output token counts only).</span>
            </li>
            <li className="flex gap-2">
              <span className="text-green-400 mt-0.5 shrink-0">✓</span>
              <span>Persist a <code className="bg-zinc-800 px-1 rounded text-xs">SessionRecord</code> with those counts into your leaderboard totals.</span>
            </li>
            <li className="flex gap-2">
              <span className="text-green-400 mt-0.5 shrink-0">✓</span>
              <span>Return the upstream response to your client — same status, same headers, same body.</span>
            </li>
          </ul>
        </section>
        ...
        <section>
          ...
          <ul className="space-y-2 text-sm text-zinc-300">
            ...
            <li className="flex gap-2">
              <span className="text-red-400 mt-0.5 shrink-0">✗</span>
              <span>Store your Anthropic API key in plaintext — it is encrypted with AES-256-GCM before it touches our database.</span>
            </li>
            <li className="flex gap-2">
              <span className="text-red-400 mt-0.5 shrink-0">✗</span>
        ...
        <section>
          ...
          <p className="text-sm text-zinc-400 mb-3">
            The proxy implementation is open source. These are the exact files that
            handle your traffic:
          </p>
          <ul className="space-y-1 text-sm font-mono text-zinc-300">
            <li>
              ...
              <span className="text-zinc-500 ml-2 font-sans text-xs">— proxy + key management routes</span>
            </li>
            <li>
              <code className="bg-zinc-800 px-2 py-0.5 rounded text-xs">
                apps/api/src/lib/ingest.ts
              </code>
              <span className="text-zinc-500 ml-2 font-sans text-xs">— session persistence (counts only)</span>
            </li>
            <li>
              <code className="bg-zinc-800 px-2 py-0.5 rounded text-xs">
                infra/migrations/0004_proxy_keys.sql
              </code>
              <span className="text-zinc-500 ml-2 font-sans text-xs">— user_anthropic_keys table schema</span>
            </li>
          </ul>
        </section>
...
ANTHROPIC_API_KEY=<your Token Rats user token>`}
          ...
          <p className="mt-3 text-xs text-zinc-500">
            The <code className="bg-zinc-800 px-1 rounded">ANTHROPIC_API_KEY</code> value here is your Token Rats
            Bearer token (from the CLI login), not your Anthropic key. Your Anthropic key is
            stored separately using the form below and sent directly to Anthropic — it never
          ...
          {user ? (
            <>
              <p className="text-sm text-zinc-400 mb-4">
                We&apos;ll use this key for all proxy requests from your account.
                You can also leave it empty to use a shared key if one is configured.
              </p>
